...
namespace DGWnd.UI
{
    public partial class frmMDI : Form
    {
        ...
        #region *********************  Menu ***************************
        private void mnuWHorizontal_Click(object sender, EventArgs e) => this.LayoutMdi(MdiLayout.TileHorizontal);
        private void mnuWVertical_Click(object sender, EventArgs e) => this.LayoutMdi(MdiLayout.TileVertical);
        private void mnuWArrange_Click(object sender, EventArgs e) => this.LayoutMdi(MdiLayout.ArrangeIcons);
        private void mnuWCascade_Click(object sender, EventArgs e) => this.LayoutMdi(MdiLayout.Cascade);
        #endregion **************** End Menu *****************

        private void frmMDI_Load(object sender, EventArgs e) => AttachNewChildForm(new UI.frmDataDefinition());
        ...
        private void CheckChildForms()
        {
            for (int i = 0; i < this.stripWnds.Items.Count; i++)
            {
                ...
                if (item.Name.StartsWith("TSB"))
                {
                    ...
                    {//circle by form collection
                        if (frm.Handle.ToInt32() == wndID)
                        { // form exist
                            flag = true; break;
                        }
                    }
        ...
        {
            this.wndCnt++;
            if (this.stripWnds.Items.Count != 0)
            { // add separator
        ...
        private void ChildClosed(object sender, System.EventArgs e)
        {
            ...
            if (this.Visible)
            {// free memory of previous form by open new blank form
                Form frm = new Form();
                frm.MdiParent = this;
                frm.Show();
                frm.Dispose();
        ...
        private void btnProxyChecker_Click(object sender, EventArgs e) => AttachNewChildForm(new ProxyChecker.Form1());
