...
namespace spMain.Comp
{
    public partial class StockGraph : ZedGraphControl
    {
        ...
        {
            if (Control.MouseButtons == MouseButtons.Left && csWinApi.IsKeyPress(Keys.T))
            {// Text
                return DrawText(e);
            }
            if (Control.MouseButtons == MouseButtons.Left && csWinApi.IsKeyPress(Keys.L))
            {// Line
                return StartDrawLine(e, false);
            }
            if (Control.MouseButtons == MouseButtons.Left && csWinApi.IsKeyPress(Keys.A))
            {// Arrow
        ...
        bool StockGraph_MouseUpEvent(ZedGraphControl sender, MouseEventArgs e)
        {
            ...
            {
                if (_drawLineObj.Location.Width == 0 && _drawLineObj.Location.Height == 0)
                {// Remove blank line
