  ...
  public partial class DataIndicator {
    ...
    public void UpdateData(int timerID) {
      if (timerID > this._lastTimerID) {
        ...
        // Update quote
        switch (this._indID) {
          case "dquote": Upd_QuoteDifferential(); break; // 
//          case "value_changes": Upd_ValueChanges(); break; // 
  //        case "value_abschanges": this.Upd_ValueAbsChanges(); break; // 
          case "dj+": Upd_DJPlus(); break; // DJ+ (ToS checked)
          case "dj-": Upd_DJMinus(); break; // DJ- (ToS checked)
          case "adx": Upd_ADX(); break; // ADX (ToS checked)
          case "roc": Upd_RateOfChange(); break; // Rate of Change  (ToS checked)
          case "rsi": Upd_RSI(); break; // RSI (old version)
          case "rsiwilder": Upd_RSIWilder(); break; // RSI Wilder (ToT)
          case "rsi_ema": Upd_RSI_EMA(); break; // RSI with EMA (toT)
          case "macd": Upd_MACD(); break;//(ToS checked)
          case "macdsignal": Upd_MACDSignal(); break;//(ToS checked)
          case "macdhistogram": Upd_MACDHistogram(); break;//(ToS checked)
          case "maenvdown": this.Upd_MAEnvelope(-1.0); break;
          case "maenvup": this.Upd_MAEnvelope(1.0); break; // 
          case "momentum": this.Upd_Momentum(); break; // ToS checked = MomentumPercent
//          case "moneyflow": this.Upd_MoneyFlow(); break; // Money Flow
          case "onbalvol": this.Upd_OnBalVolume(); break;// On Balance Volume (ToS checked)
          case "singleline": this.Upd_SingleLine(); break;
          case "%r": this.Upd_R(); break;// Williams, R% (ToS checked)
          case "ma": this.Upd_MA(); break; // moving average of quote (ToS checked)
          case "stdev": this.Upd_StDev(); break; // Standart deviation of quote (ToS checked)
          case "bb": this.Upd_BB(); break; // Middle line of Bollinger bands (ToS checked)
          case "bbu": this.Upd_BB_Bands(1); break; // Bollinger upper band (ToS checked)
          case "bbl": this.Upd_BB_Bands(-1); break; // Bollinger lower band (ToS checked)
          case "%d": this.Upd_D(); break;// stochastic (ToS checked)
          case "%k": this.Upd_K(); break;// stochastic (ToS checked)
          case "%kfast": this.Upd_KFast(); break;// stochastic (ToS checked)
          case "framedquote": this.Upd_FramedQuote(); break;
          case "quoteother": this.Upd_QuoteOther(); break;
          case "compare": this.Upd_Compare(); break;
          case "max": this.Upd_Max(); break;
          case "min": this.Upd_Min(); break;
          case "updowndoji": this.Upd_UpDownDoji(); break;
          case "volbuy_ma": this.Upd_VolBuySell_MA("volbuy"); break;
          case "volsell_ma": this.Upd_VolBuySell_MA("volsell"); break;
//          case "volbuy_ma":
  //        case "volsell_ma": this.Upd_VolBuySell_MA(); break;
          case "volsell": this.Upd_VolSell(); break;
          case "volbuy": this.Upd_VolBuy(); break;
          case "volume": this.Upd_Volume(); break;
...
//      DataInput di = (DataInput)this._inputs["doji_k"];
      ...
      for (int i =this._startItemNo; i < this._endItemNo; i++) {
        ...
        if (q.open < q.close * (1 -1 / doji_k)) {// uptick
          this._data[i] = 1;
        }
        else if (q.open > q.close * (1 + 1 / doji_k)) {// downtick
          this._data[i] = 2;
        }
        else this._data[i] = 3;
      }
