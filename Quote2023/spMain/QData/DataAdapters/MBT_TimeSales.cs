...
namespace spMain.QData.DataAdapters {
  ...
  [Serializable]
  class MBT_TimeSales : Data.DataAdapter {
    ...
    public override IList GetData(List<object> inputs, int lastDataOffset, out int newDataOffset) {
      ...
      List<MbtTickHttp> data = new List<MbtTickHttp>();
    ...
    void LoadData(string symbol, DateTime date, List<MbtTickHttp> data) {
      ...
      if (File.Exists(fn)) {
        ...
        if (bytes != null) {
          ...
          data.AddRange((List<DataFormat.MbtTickHttp>)chunk.GetData(bytes));
        }
      }
    }
