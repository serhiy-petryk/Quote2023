// Taken from http://www.codeproject.com/KB/dotnet/FastSerializer.aspx
// Remove the DEBUG condition if you want to always check for not optimizable values at the small expense of runtime speed
// Serializator types:
//  - CompactFormatter = require ISerializable realization for each object 
//  - FastSerialization (by Tim Haynes) = using BinaryReader/Writer +  ??? object[] in ISerialization interface
//  - FastSerializer (this) = can use instead of BinarySerialization + can use object[] in ISerialization
//  - RawSerializer (by Clifton) = no Dictionary Support and others. There are some examples (Crypto + Compress + DataTable + ...)
...
/*    public virtual void GetObjectData(SerializationInfo info, StreamingContext context) {
    ...
    protected ClassConstructor(SerializationInfo info, StreamingContext context) {
...
/*    
1. More Compact. Placed object stream 
 * public virtual void GetObjectData(SerializationInfo info, StreamingContext context) {
      csFastSerializer.Serialize1(new object[] {myInt32Field, myInt32Field, ...});
    }
    protected ClassConstructor(SerializationInfo info, StreamingContext context) {
      SerializationReader reader = csFastSerializer.GetReaderinfo);
      myInt32Field = reader.ReadInt32();
      myDateTimeField = reader.ReadDateTime();
      myString = reader.ReadString();
      reader.Close()
    }
2. Более удобная. Placed object[] to stream
 * public virtual void GetObjectData(SerializationInfo info, StreamingContext context) {
      csFastSerializer.Serialize(new object[] {myInt32Field, myInt32Field, ...});
    }
    protected ClassConstructor(SerializationInfo info, StreamingContext context) {
      object[] oo  = csFastSerializer.Deserialize(info);
      myInt32Field = (int)oo[0];
      myDateTimeField = (DateTime)oo[1];
    }*/
...
namespace spMain.csFastSerializer {

  public static class Utils {// Added by Sergei Petrik

    public static void Test() {
      byte[] bb =new byte[] {1,2,3,4};
      MemoryStream ms = new MemoryStream();
      SerializationWriter sw = new SerializationWriter();
//      sw.Write(new object[] { "1234".ToCharArray(), bb });
      sw.Write("1234");
      sw.Write(bb);

      byte[] b = sw.ToArray();
      SerializationReader sr = new SerializationReader(b);
//      char[] cc= "sdfdgf".ToCharArray()
      string s1 = sr.ReadString();
      byte[] bb2 = sr.ReadByteArray();
    ...
    // 1. More compact
    public static void Serialize1(System.Runtime.Serialization.SerializationInfo info, object[] data) {
      SerializationWriter writer = new SerializationWriter();
      foreach (object o in data) {
        writer.WriteObject(o);
      }
      info.AddValue("_", writer.ToArray());
      writer.Close();
    }
    public static SerializationReader GetReader(System.Runtime.Serialization.SerializationInfo info) {
      byte[] bb = null;
      try { bb = (byte[])info.GetValue("_", typeof(byte[])); }
      catch { }
      if (bb == null) {
        bb = (byte[])info.GetValue("data", typeof(byte[])); // old format
      }
      return new SerializationReader(bb);
    }
    // 2. More удобная
    public static void Serialize(System.Runtime.Serialization.SerializationInfo info, object[] data) {
      SerializationWriter writer = new SerializationWriter();
      writer.Write(data);
      info.AddValue("_", writer.ToArray());
      writer.Close();
    }
    public static object[] Deserialize(System.Runtime.Serialization.SerializationInfo info) {
      byte[] bb = null;
      try { bb = (byte[])info.GetValue("_", typeof(byte[])); }
      catch { }
      if (bb == null) {
      ...
      SerializationReader reader = new SerializationReader(bb);
      object[] x = reader.ReadObjectArray();
      reader.Close();
      return x;
    }
    public static object CheckFastSerialization(object o) {// Serialize + deserialize object
      SerializationWriter objectWriter = new SerializationWriter();
      objectWriter.OptimizeForSize = true;
      objectWriter.WriteObject(o);

      SerializationReader objectReader = new SerializationReader(objectWriter.ToArray());
      return objectReader.ReadObject();
    }
    public static object CheckBinSerialization(object o) {// Serialize + deserialize object
    ...
    // =================   Common utilities ========================
    public static object File_ReadObject(string filename) {
      if (File.Exists(filename)) {
        SerializationReader objectReader = new SerializationReader(File.ReadAllBytes(filename));
    ...
    public static void File_SaveObject(object o, string filename) {
      string tmpFN = filename + ".tmp";
      if (File.Exists(tmpFN)) File.Delete(tmpFN);
      Stream stream = File.Open(tmpFN, FileMode.CreateNew);

      SerializationWriter objectWriter = new SerializationWriter();
      objectWriter.OptimizeForSize = true;
      objectWriter.WriteObject(o);
      byte[] bytes = objectWriter.ToArray();
      stream.Write(bytes, 0, bytes.Length);
      stream.Close();
      objectWriter.Close();
      if (File.Exists(filename)) File.Delete(filename);
      File.Move(tmpFN, filename);
      return;
    ...
    public static object GetClone(object o) {
      ...
      SerializationReader objectReader = new SerializationReader(objectWriter.ToArray());
      object clone = objectReader.ReadObject();
      objectWriter.Close();
      objectReader.Close();
      return clone;
      ...
      bf.Serialize(stream, o);
      stream.Position = 0;
      object newObject = bf.Deserialize(stream);
      stream.Close();
      return newObject;
    }
  }
