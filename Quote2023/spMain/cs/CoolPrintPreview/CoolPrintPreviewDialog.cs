  ...
  internal partial class CoolPrintPreviewDialog : Form {
    ...
    void _btnPrint_Click(object sender, EventArgs e) {
      using (PrintDialog dlg = new PrintDialog()) {
        ...
        // show dialog
        _toolStrip.Capture = false; // Bug fix by SP
    ...
    void _btnPageSetup_Click(object sender, EventArgs e) {
      using (PageSetupDialog dlg = new PageSetupDialog()) {
        dlg.Document = Document;
        dlg.EnableMetric = true;// Bug fix by SP
        if (dlg.ShowDialog(this) == DialogResult.OK) {
    ...
    private void toolStripButton1_Click(object sender, EventArgs e) {
      PageImageList il = this._preview.PageImages;
      il[0].Save(@"c:\xxx1.bmp");
    }
  }
}
